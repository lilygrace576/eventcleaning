...
int main(int argc, char **argv){
    ...
    TH1F* h1 = new TH1F("h1", "Average Amplitude Distribution", 300, 0, 1500);  //Test
    ...
    TH1F* h4 = (TH1F*)h1->Clone("h4");                                          //HLED
    // h1->SetStats(0); 

    TH1F* h2 = new TH1F("h2", "Avg Amplitude RMS", 100, 0, 100);        //Test
    ...
    // for(int f = 65; f<90; f++){
        ...
        ////
            ...
            // checking for correct event classification with flasher cutoff

                if (WhichEvent == "Test"){
                    //Test Branch actually HLED
                    if (util->GetEventAmplitudeSum(Amplitudes)/MaxNofChannels >= FlasherEventsCutOff){  // > cutoff = HLED
                        // cout << "avg amp: " << util->GetEventAmplitudeSum(Amplitudes)/MaxNofChannels << endl;
                        hled_event_counter +=1;
                        h4->Fill(util->GetEventAmplitudeSum(Amplitudes)/MaxNofChannels);    //fill HLED plots
                        h3->Fill(RMS);
                        for(int j = 0; j<MaxNofChannels; j++){
                            TotalAmplitudeValues[j] = TotalAmplitudeValues[j]+Amplitudes[j];
                        }
                    } else{                                                                             // < cutoff = Test
                        //Test Branch actually Test
                        h1->Fill(util->GetEventAmplitudeSum(Amplitudes)/MaxNofChannels);    //fill Test plots
                        if (RMS < 2){
